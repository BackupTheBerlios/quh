...
/* See ym2151.h for copyright information */
#include "ym2151.h"

//extern int channels;
//int isbitset(void *, int);

static void YM2151ResetChip(int num);
...
static void init_chip_tables(YM2151 *chip)
{
        ...
        for (i=0; i<1*12*64; i++)
        {
                ...
                chip->freq[i] = (unsigned)(phaseinc*mult);
                for (j=1; j<11; j++)
                {
        ...
        for (i=0; i<256; i++)
        {
                ...
                /*calculate phase increment*/
                chip->LFOfreq[0xff-i] = (unsigned)(( (pom*LFO_LEN) / (double)chip->sampfreq ) * mult); /*fixed point*/
        }
        ...
        for (i=4; i<64; i++)
        {
                ...
                chip->A_Time[i] = (unsigned)pom;
                chip->D_Time[i] = (unsigned)pom2;
        }
        ...
        for (i=0; i<1024; i++)
        {
                ...
                #else
                        chip->TimerA[i] = (unsigned)(pom * (double)chip->sampfreq * mult);  /*number of samples that timer period takes (fixed point) */
                #endif
        }
        for (i=0; i<256; i++)
        {
                ...
                #else
                        chip->TimerB[i] = (unsigned)(pom * (double)chip->sampfreq * mult);  /*number of samples that timer period takes (fixed point) */
                #endif
        }
...
INLINE static void envelope_KONKOFF(OscilRec * op, int v)
{
        if (v&8)
        {
                if (!op->key){
...
INLINE static void refresh_EG( YM2151 *chip, int chan)
{
        OscilRec * op;
        unsigned int kc, v;
...
void YM2151WriteReg(int n, int r, unsigned v)
{
        YM2151 *chip = &(YMPSG[n]);
...
int YM2151Init(int num, int clock, int rate)
{
        ...
        if (YMPSG) return 0x20000000; /* duplicate init. */

        YMNumChips = num;

        YMPSG = (YM2151 *)malloc(sizeof(YM2151) * YMNumChips);
        if (YMPSG == NULL) return(errno);
        ...
        init_tables();
        for ( i=0 ; (unsigned)i<YMNumChips; i++ )
        {
                YMPSG[i].clock = clock;
...
INLINE static unsigned int volume_calc(OscilRec *op)
{
        op->phase+=op->freq;
