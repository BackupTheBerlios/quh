...
#define DATACONV(A) (DATATYPE)((A) / STEP)
	int i;
...
//		if(isbitset(&channels, 14))
		    out = vol[0] * R->Volume[0];
//                else
//		    out = 128;
//                if(isbitset(&channels, 15))
		    out += vol[1] * R->Volume[1];
//                else
//		    out += 128;
//		if(isbitset(&channels, 16))
		    out += vol[2] * R->Volume[2];
//                else
//		    out += 128;
//		if(isbitset(&channels, 17))
		    out += vol[3] * R->Volume[3];
//                else
//		    out += 128;
...
static void SN76496_set_clock(int chip,int clock)
{
	...
	/* number. */
	R->UpdateStep = (unsigned)(((double)STEP * R->SampleRate * 16) / clock);
}
