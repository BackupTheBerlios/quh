...
static int
quh_decode_probe (const char *fname, const int id)
{
  int probe_id[FILTER_MAX];
    
//  probe_id[0] = QUH_READ_IN;
//  probe_id[1] = QUH_DECODE_PASS;
  probe_id[0] = id;
  probe_id[1] = 0;
  ...
  if (!filter_read (quh.filter_chain, &quh.demux))
    {
//      quh_decodeer_sanity_check (&quh.demux);
...
quh_decode_open (st_quh_filter_t *file)
{
    ...
    if (*suffix)
      {
        ...
        {
          if (!quh_decode_probe (quh.fname[quh.current_file], f->id))
            {
              id = f->id;
              found = 1;
            }
  ...
  // decode by file content
  if (!found)
    for (x = 0; quh_filter_write[x]; x++)
      if (quh_filter_write[x]->read) // we use the read prototype for decodeing
        if (!quh_decode_probe (quh.fname[quh.current_file], quh_filter_write[x]->id))
          {
...
static int
quh_decode_ctrl (st_quh_filter_t *file)
{
  int x = 0;
  char buf[MAXBUFSIZE];
